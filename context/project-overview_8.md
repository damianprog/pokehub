...
// NOTE: No `Session` model — JWT session strategy (see §14). The PrismaAdapter
// persists User, Account, and VerificationToken only; sessions live in a signed
// HttpOnly JWT cookie, not the DB. Add a Session model back if switching to
// database sessions (which would drop Credentials support).
...
| `User.password` (nullable, hashed)                       | Credentials sign-in path. Null for OAuth / magic-link users. Hashed app-side (argon2/bcrypt) in the registration route.                        |
...
| Auth         | NextAuth v5 (JWT sessions)         | OAuth (Google, GitHub) + magic link + email/password          |
...
# NextAuth (AUTH_* prefix — v5 auto-infers provider credentials from this naming)
AUTH_SECRET="..."
AUTH_GITHUB_ID="..."
AUTH_GITHUB_SECRET="..."
AUTH_GOOGLE_ID="..."
...
| **Auth & subscription**            | NextAuth v5, JWT sessions — OAuth + magic link + email/password (Credentials); Stripe webhook handling, feature flag (`isPro` + cosmetic-only enforcement) |
...
Three sign-in paths: OAuth (GitHub, Google), magic link (Email provider), and email/password (Credentials provider). Credentials adds password-storage and reset-flow surface area — accepted deliberately, because implementing it end-to-end (hashing with argon2/bcrypt, a custom `authorize` handler, a registration route, and a reset flow) is a stronger portfolio signal than an OAuth-only setup. Magic link stays as the passwordless option.

**Session strategy: JWT, not database sessions.**
The Credentials provider _only_ works with the JWT strategy: Auth.js does not persist credentials-authenticated users to a `Session` table and throws `UnsupportedStrategy: Signing in with credentials only supported if JWT strategy is enabled` under `session: { strategy: "database" }`. Since email/password is in scope, the whole app runs on `session: { strategy: "jwt" }`. Consequences, accepted knowingly:

- The `Session` model is dropped. The PrismaAdapter still persists `User`, OAuth `Account`, and magic-link `VerificationToken` rows — the session itself lives in a signed, encrypted, HttpOnly JWT cookie, not the DB.
- "Sign out everywhere" and per-device session listing are no longer free. A JWT can't be revoked before its expiry without a server-side blocklist; mitigated with short session lifetimes (and rotation). This is the reverse of the earlier database-session rationale — the trade was made to unlock email/password.
- `proxy.ts` can read the session straight from the cookie without a DB roundtrip, which suits the Node/edge middleware path.

**Account linking across three providers.**
The same email arriving via OAuth, magic link, and password must resolve to one `User`. Auth.js does _not_ auto-link an OAuth sign-in to a pre-existing email-based account by default (anti-hijack), so linking is handled explicitly on verified emails rather than by flipping `allowDangerousEmailAccountLinking`. Credentials users are written to `User` (with a hashed `password`) by the registration route, then looked up in `authorize`; OAuth and magic-link users are created/linked by the adapter.
